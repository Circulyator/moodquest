...
function renderOnboardingScreen() {
  ...
  adoptPetForm.addEventListener('submit', async (event) => {
    ...
    try {
      const existingPet = await fetchMyPet();

      if (existingPet) {
      showToast('У тебя уже есть питомец', 'error');
      await routeApp();
      return;
    }
...
async function renderPatientHomeScreen(pet) {
  ...
  const petSlug = pet.pet_types?.slug || null;

if (!petSlug) {
  showToast('Перезагружаем питомца...', 'info');
  setTimeout(() => routeApp(), 300);
  return;
}
